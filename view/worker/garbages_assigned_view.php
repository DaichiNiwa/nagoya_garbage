...
<head>
  ...
  <title>担当地域のごみ一覧/名古屋市粗大ごみ</title>
</head>
...
<body>
  ...
  <div class="container">
    <h1>あなたの担当地域のごみ（作業員用）</h1>
    <p>担当地域: <?php print h(area_number_to_name($worker['area'])) ?></p>
    <?php include VIEW_PATH . 'templates/messages.php'; ?>

    <nav aria-label="Page Navigation">
      <ul class="pagination row text-center">
        <li class="col-1 p-0 page-item <?php if($current_page <= 1){ print h('disabled');}?>">
          <a  class="page-link" href="<?php print h(WORKER_ASSIGNED_URL . '?current_page=' . ($current_page - 1)) ?>" aria-label="Previous Page">
            <span aria-hidden="true">&laquo;</span>
          </a>
        </li>
        <?php for($i = 1; $i <= $total_pages_number; $i++){ ?>
          <li class="col-1 p-0 page-item <?php if($current_page === $i){ print h('active');}?>">
            <a class="page-link" href="<?php print h(WORKER_ASSIGNED_URL . '?current_page=' . $i) ?>"><?php print h($i) ?></a>
          </li>
        <?php } ?>
        <li class="col-1 p-0 page-item <?php if($current_page >= $total_pages_number){ print h('disabled');}?>">
          <a href="<?php print h(WORKER_ASSIGNED_URL . '?current_page=' . ($current_page + 1)) ?>"  class="page-link" aria-label="Next Page">
    ...
    <?php if($assigned_garbages_amount > 0){ ?>
